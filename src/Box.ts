...
export class Box {
  ...
  get periphery() {
    let atAngle = (angle: number) => {
      let halfWidth = this.width / 2;
      let halfHeight = this.height / 2;

      // the radius of the circle tangent to the corners of the box
      ...
      // the distance from the center of the box that the point on the periphery of the box will be
      ...
      if (Math.abs(this.centerX - pointOnCircle.x) > halfWidth) {
        ...
        // can become nonfinite when angle is close to 90 or 270 degrees
      ...
      if (Math.abs(this.centerY - pointOnCircle.y) > halfHeight) {
        ...
        // can become nonfinite when angle is close to 0 or 180 degrees
    ...
    return {
      /**
       * Returns the point on the periphery of the box at the given angle.
       */
      atAngle,
    };
  }
